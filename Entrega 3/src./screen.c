...
#define TOTAL_DATA (ROWS * COLUMNS) + 1               /*!< Total data */
  ...
  #define BG_CHAR '~'                                 /*!< Virgulilla character (~) */
#define FG_CHAR ' '                                   /*!< Space character ( ) */
#define PROMPT " prompt:> "                           /*!< Character string ( prompt:> ) */

#define ACCESS(d, x, y) (d + ((y) * COLUMNS) + (x))   /*!< Access to d+y*Columnas+x */
 ...
 */
struct _Area {
  int x, y, width, height;   /*!< Coordinate x, Coordinate y, Width, Height */
  char *cursor;              /*!< Cursor */
};
...
/**
 * @brief Identify if the cursor is out of bounds.
 * @param area Area in which you want to identify the cursor.
 * @return integer to the ACCESS position of the cursor.
 */
int  screen_area_cursor_is_out_of_bounds(Area* area);
/**
 * @brief Scroll the screen up.
 * @param area Area you want to move.
 */
